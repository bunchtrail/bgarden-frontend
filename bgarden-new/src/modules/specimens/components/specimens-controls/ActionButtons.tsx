...
const ActionButtons: React.FC<ActionButtonsProps> = ({ 
  ...
  return (
    <div className={`flex ${variant === 'card' ? 'flex-wrap gap-3' : 'justify-end space-x-2'}`}>
      <Button
        variant="neutral"
        size="small"
        onClick={() => navigate(`/specimens/${specimenId}`)}
        className={`flex items-center !px-3 py-1.5 rounded-md shadow-sm hover:shadow-md ${animationClasses.transition}`}
      >
      ...
      <Button
        ...
        className={`flex items-center !px-3 py-1.5 rounded-md shadow-sm hover:shadow-md group ${animationClasses.transition}`}
      >
