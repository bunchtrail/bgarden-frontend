...
export const useMapData = (options?: {
  ...
  const checkIfEmpty = useCallback(
    (regionsData: RegionData[] = [], plantsData: any[] = []) => {
      const hasRegions =
        regionsData && Array.isArray(regionsData) && regionsData.length > 0;
      const hasPlants =
        plantsData && Array.isArray(plantsData) && plantsData.length > 0;
    ...
    async (forceUpdate = false) => {
      ...
      try {
        ...
        if (
          ...
          const areasData = regionBridge.regionsToAreas(
            regionsData,
            mapConfig.mapType
          );
          setAreas(areasData);
